...
with col2:
    # Novo filtro de ano adicionado
    all_years = sorted(df_exp['CO_ANO'].unique().tolist(), reverse=True)
...
df_exp_filtered_sc = df_exp[
    (df_exp['CO_ANO'] == selected_year) & # Usa o ano selecionado
    (df_exp['SG_UF_NCM'].isin(selected_ufs))
]

df_imp_filtered_sc = df_imp[
    (df_imp['CO_ANO'] == selected_year) & # Usa o ano selecionado
